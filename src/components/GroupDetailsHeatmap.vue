...
} from "@/d3Viz/createGroupDetailsHeatmap";
import type { AxisItem, RadarDog } from "@/d3Viz/createRadarChart";
...
onMounted(() => {
  chart = createGroupDetailsHeatmap(svgRef.value!, {
    onHover: (cell, ev) => {
      hovered.value = cell;
      tip.value.show = true;
      setTipFromEvent(ev);
    ...
    onClick: (rowIndex) => emit("toggleFocus", rowIndex),
  });
